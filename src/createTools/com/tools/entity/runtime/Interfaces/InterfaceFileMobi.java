...
public class InterfaceFileMobi extends InterfaceFileBase {

	@Override
	public void interExcel(File path, List<DBTable> tables)
			throws WriteException, IOException {
		StringBuffer mobiHeadersParamers = new StringBuffer();
		mobiHeadersParamers.append("author_key").append(":").append("登陆后返回验证码").append("\n");
		mobiHeadersParamers.append("request-type").append(":").append("请求来源，值为：ios或者android").append("\n");
		mobiHeadersParamers.append("Content-Type").append(":").append("格式类型，值为：application/json").append("\n\n");
		...
		String f = TemplateFile.realPathBuild("mobiapi_readme_" + Start.pmanRequestHost.replace("http://", "").replace(":8080", "").replaceAll("\\.", "_"),"",path.getPath());		
		//创建工作薄
		...
		for(String keySet:sheetMap.keySet()){
			...
			sheet.addCell(new Label(3,0,"header中参数",titleFormate));
			sheet.addCell(new Label(4,0,"提交参数",titleFormate));
			sheet.addCell(new Label(5,0,"参数说明",titleFormate));
		...
		for(DBTable table:tables){
			...
			sheet.addCell(new Label(beginColumn + 2,beginRow,Start.pmanRequestHost + "/apimobi/" + CreateUtils.getTab(table.getCode()).toLowerCase() + "/save",titleNoBold));
			sheet.addCell(new Label(beginColumn + 2,beginRow + 1,Start.pmanRequestHost + "/apimobi/" + CreateUtils.getTab(table.getCode()).toLowerCase() + "/{主键值}",titleNoBold));
			sheet.addCell(new Label(beginColumn + 2,beginRow + 2,Start.pmanRequestHost + "/apimobi/" + CreateUtils.getTab(table.getCode()).toLowerCase() + "",titleNoBold));
			sheet.addCell(new Label(beginColumn + 2,beginRow + 3,Start.pmanRequestHost + "/apimobi/" + CreateUtils.getTab(table.getCode()).toLowerCase() + "/del",titleNoBold));
			
			sheet.mergeCells(beginColumn + 3, beginRow, beginColumn + 3, beginRow + 4);
			sheet.addCell(new Label(beginColumn + 3,beginRow,mobiHeadersParamers.toString(),titleNoBold));
			
			sheet.addCell(new Label(beginColumn + 4,beginRow,"{\"data\":" + DBPdmUtils.genTableJson(table,true) + "}",titleNoBold));
			sheet.addCell(new Label(beginColumn + 4,beginRow + 1,"",titleNoBold));
			sheet.addCell(new Label(beginColumn + 4,beginRow + 2,"{\"data\":" + DBPdmUtils.genTableJson(table,false) + ",\r\"page\":" + page.toJsonString() + "\r}",titleNoBold));
			sheet.addCell(new Label(beginColumn + 4,beginRow + 3,"{\"data\":\r{" + ToTemplateFileGenUtils.getDBTabelPrimaryKeyJsonString(table) + "\r}\r}",titleNoBold));
			
			sheet.mergeCells(beginColumn + 5, beginRow, beginColumn + 5, beginRow + 4);
			StringBuffer clm = new StringBuffer();
			for(DBColumn dbc:table.getCols()){
				clm.append(CreateUtils.getCol(dbc.getCode()) + ":" + dbc.getName() + "\n");
			}
			sheet.addCell(new Label(beginColumn + 5,beginRow,clm.toString(),titleNoBold));
		}
		for(String keySet:sheetMap.keySet()){
			sheetIndex += 1;
	...
	@Override
	public void interPostMan(File path, List<DBTable> tables, String webname)
			throws Exception {
		PManJson pmJson = new PManJson();
		pmJson.setId(UUID.randomUUID().toString());
		pmJson.setName(webname + " mobi " + Start.pmanRequestHost.replace("http://", "").replace(":8080", ""));
		pmJson.setDescription(webname + " mobi 接口测试，" + Start.pmanRequestHost.replace("http://", "").replace(":8080", ""));
		...
		for(DBTable table:tables){
			...
			//显示保存接口
			requests.add(new ControllerMethodSave(table).genInterfaceMobiPMRequest(pmJson.getId(), "{\"data\":" + DBPdmUtils.genTableJson(table, true) + "}", lastFolderId));
			//显示单个接口
			requests.add(new ControllerMethodShow(table).genInterfaceMobiPMRequest(pmJson.getId(),"{\"data\":{\n\t" + ToTemplateFileGenUtils.getDBTabelPrimaryKeyJsonString(table) + "\n\t}\n}", lastFolderId));
			//显示列表接口
			Pagination page = new Pagination();
			requests.add(new ControllerMethodList(table).genInterfaceMobiPMRequest(pmJson.getId(),"{\"data\":" + DBPdmUtils.genTableJson(table, false) + ",\n\"page\":" + page.toJsonString() + "}", lastFolderId));
			//显示删除接口
			requests.add(new ControllerMethodDel(table).genInterfaceMobiPMRequest(pmJson.getId(),"{\"data\":{\n\t" + ToTemplateFileGenUtils.getDBTabelPrimaryKeyJsonString(table) + "\n\t}\n}", lastFolderId));
		}
        ...
        String f = TemplateFile.realPathBuild("mobiapi_postman_" + Start.pmanRequestHost.replace("http://", "").replace(":8080", "").replaceAll("\\.", "_"),"",path.getPath());			
	...
	public void interJsp(File path, List<DBTable> tables) throws Exception {
		...
		for(DBTable table:tables){
			Pagination page = new Pagination();
			//保存接口json格式
			jsContent.append(new ControllerMethodSave(table).genInterfaceMobiLocalStorage("{\"data\":" + DBPdmUtils.genTableJson(table, true) + "}"));
			//列表接口json格式
			jsContent.append(new ControllerMethodList(table).genInterfaceMobiLocalStorage("{\"data\":" + DBPdmUtils.genTableJson(table, false) + ",\npage:" + page.toJsonString() + "}"));
			//显示单个接口json格式
			jsContent.append(new ControllerMethodShow(table).genInterfaceMobiLocalStorage("{data:{" + ToTemplateFileGenUtils.getDBTabelPrimaryKeyJsonString(table) + "}}"));
			//删除接口json格式
			jsContent.append(new ControllerMethodDel(table).genInterfaceMobiLocalStorage("{data:{" + ToTemplateFileGenUtils.getDBTabelPrimaryKeyJsonString(table) + "}}"));
			
		}
		String f = TemplateFile.realPathBuild("mobiapi","",path.getPath());			
		...
		TemplateFile.process(TemplateFile.MOBI_INTERFACE_JSON, out, root);
		
		String fLs = TemplateFile.realPathBuild("mobiapi_localStorage","",path.getPath());			
